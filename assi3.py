class CourseHistory:
    ...
    # 1_3 calculation
    def final_calc(self):
        print(f"제출용:{CourseHistory.submit_time} 학점(GPA: {CourseHistory.submit_gpa/CourseHistory.submit_time})")
        print(f"제출용:{CourseHistory.archive_time} 학점(GPA: {CourseHistory.archive_gpa/CourseHistory.archive_time})")
    def numtwofunc(self):
        for element in self.taken_course_list:
            print(f"[{self.course_id[element[0]]}] {element[1]} 학점: {element[2]}")

    def numthreefunc(self):
        search = str(input("조회할 과목을 입력하시오 :"))
        for element in self.taken_course_list:
            if search == self.course_id[element[0]]:
                print(f"[{search}] {element[1]} 학점: {element[2]}")
                break
        else:
            print("해당하는 과목이 없습니다.")
    def save_file(self):
        file=open('GPA file.txt','w')
        for element in self.taken_course_list:
            file.write(f"[{self.course_id[element[0]]}] {element[1]} 학점: {element[2]}\n")
        file.close()
...
while 1:
    ...
    print('5. 파일 저장')
    print('6. 종료')
    # 사용자 입력(1 or 2 or 3 or 4 or 5 or 6)
